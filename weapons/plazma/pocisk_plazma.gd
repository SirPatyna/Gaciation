extends CharacterBody3D

@export var team = -1
const GRAVITY = 15.34
@onready var ptr = Engine.physics_ticks_per_second
var bounces_left:int = 3
var initial_rotation: Vector3
var direction: Vector3
var add_speed: Vector3
var damage:int
var projectile_speed: float
var sprite_change:bool = false
var sprite_frame:int = 3
const sprite = preload("res://weapons/plazma/pocisk_plazma.png")
const sprite_alt = preload("res://textures/snowball.png")


func _on_area_3d_body_entered(body: Node3D) -> void:
	if not "player" in body:
		return
	if body.team != team:
		body.receive_damage.rpc_id(body.get_multiplayer_authority(), damage)
		self.queue_free()

func _ready() -> void:
	rotation = initial_rotation
	direction -= transform.basis.z 
	velocity = (Vector3(projectile_speed,projectile_speed,projectile_speed) * direction.normalized()) + add_speed
	
	

func _physics_process(_delta: float) -> void:
	if sprite_frame == 0:
		sprite_frame = 4
		sprite_change = false
		$Sprite3D.scale = Vector3(7.5, 7.5, 7.5)
		$Sprite3D.texture = sprite
	if not is_on_floor(): velocity.y -= GRAVITY * 0.65 / ptr
	
	var collision = move_and_collide(velocity / ptr)
	
	if position.y <= -50: self.queue_free()
	
	if collision:
		bounces_left -= 1
		if bounces_left <= 0: self.queue_free()
		$bounce_sound.play()
		velocity = velocity.bounce(collision.get_normal())
		$Sprite3D.texture = sprite_alt
		$Sprite3D.scale = Vector3(4, 4, 4)
		sprite_change = true
	
	if sprite_change:
		sprite_frame -=1
